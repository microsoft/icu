...
def generate_tree(
    ...
    # MSFT-Change: Work around command-line limits on Windows.
    # General a temporary file that contains the list of input files for genrb
    ...
    requests += [
        # MSFT-Change: Work around command-line limits on Windows.
        # Notes: Using a SingleExecutionRequest here instead of RepeatedOrSingleExecutionRequest, in order to
        # move the decision about whether or not to concatenate input arguments here, rather than later on dynamically.
        # We need the decision to be here so that we can force it to add a DepTarget here for the TmpFile.
        # We don't want to modify RepeatedOrSingleExecutionRequest generally speaking though, as not all ICU build tools
        # understand reading from a fileList, only the genrb tool.
        SingleExecutionRequest(
            name = "%s_res" % sub_dir,
            category = category,
            dep_targets = dep_targets + [DepTarget(input_file_list_target_name)],
